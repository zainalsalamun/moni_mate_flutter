...
class _CreateGoalPageState extends State<CreateGoalPage> {
  ...
  double get _monthlySaving =>
      _monthsRemaining > 0 ? (_targetAmount / _monthsRemaining) : 0;

  void _confirmDelete() {
    Get.defaultDialog(
        title: 'Hapus Goal?',
        middleText:
            'Apakah Anda yakin ingin menghapus target ini? Riwayat tabungan juga akan ikut terhapus.',
        textCancel: 'Batal',
        textConfirm: 'Hapus',
  ...
  void _submit() {
    if (_targetAmount <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Target nominal tidak boleh kosong'),
          backgroundColor: Colors.red));
      return;
    }
    if (_targetDate == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Silakan tentukan target waktu'),
          backgroundColor: Colors.red));
      return;
    }
    ...
    if (widget.editGoal != null) {
      widget.editGoal!.title =
          _titleController.text.isEmpty ? 'Target Baru' : _titleController.text;
      ...
      Get.back();
      messenger.showSnackBar(const SnackBar(
          content: Text('Goal berhasil diperbarui!'),
          backgroundColor: Colors.green));
    } else {
            ...
            Container(
              ...
              child: SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: _submit,
                  child: Text(
                      widget.editGoal != null ? 'Perbarui Goal' : 'Simpan Goal',
                      style: const TextStyle(
                          fontSize: 16,
                          fontWeight: FontWeight.bold,
                          color: Colors.white)),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('Berapa jumlah yang ingin kamu capai?',
                        style:
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Target besar, semangat!',
                          style: TextStyle(
                              color: Colors.blue[700],
                              fontWeight: FontWeight.bold,
                              fontSize: 12)),
                      Text('Mulai dari langkah kecil untuk hasil yang besar.',
                          style:
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Target Waktu',
                        style: TextStyle(
                            fontWeight: FontWeight.bold, fontSize: 16)),
                    Text('Kapan kamu ingin mencapai target ini?',
                        style:
                            TextStyle(color: Colors.grey[600], fontSize: 12)),
                  ],
                ),
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Waktu tersisa: $_monthsRemaining bulan',
                            style: const TextStyle(
                                color: Colors.green,
                                fontWeight: FontWeight.bold,
                                fontSize: 12)),
                        Text(
                            'Dari hari ini (${DateFormat('dd MMM yyyy').format(DateTime.now())})',
                            style: TextStyle(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('Perkiraan perhitungan berdasarkan target kamu',
          ...
          Row(
            children: [
              Expanded(
                  child: _previewData(
                      'Target', 'Rp ${_currencyFormat.format(_targetAmount)}')),
              Expanded(child: _previewData('Waktu', '$_monthsRemaining bulan')),
              Expanded(
                  child: _previewData('Kekurangan',
                      'Rp ${_currencyFormat.format(_targetAmount)}')),
            ],
          ),
          const SizedBox(height: 16),
          Container(
              ...
              children: [
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('Kamu perlu menabung:',
                          style:
                              TextStyle(color: Colors.white70, fontSize: 12)),
                      const SizedBox(height: 4),
                      Text(
                          'Rp ${_currencyFormat.format(_monthlySaving)} / bulan',
                          style: const TextStyle(
                              color: Colors.white,
                              fontWeight: FontWeight.bold,
                              fontSize: 18)),
                      const SizedBox(height: 4),
                      const Text('Agar target tercapai tepat waktu',
                          style:
